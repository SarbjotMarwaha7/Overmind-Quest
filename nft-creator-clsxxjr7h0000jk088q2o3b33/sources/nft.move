...
module overmind::NonFungibleToken {
    ...
    */
    fun init(ctx: &mut TxContext) {
        // Create a new MinterCap object
    let minter_cap = MinterCap {
        id: get_sender(),
        sales: Balance { value: 0 },
    };

    // Store the MinterCap object
    save::<MinterCap>(&minter_cap); 
    }
    ...
    */
    ...
    if payment_coin.value < 1 {
        abort("Insufficient payment for NFT minting.");
    }
    
    // Create a new NonFungibleToken object
    let nft = NonFungibleToken {
        id: generate_uid::<NonFungibleToken>(),
        name: String::from_utf8(nft_name).unwrap(),
        description: String::from_utf8(nft_description).unwrap(),
        image: Url::from_utf8(nft_image).unwrap(),
    };
    
    // Transfer the NFT to the recipient
    transfer::transfer::<NonFungibleToken>(&recipient, &nft);
    // Increment sales balance in MinterCap
    minter_cap.sales.value += 1;
    
    // Return any change
    payment_coin.value -= 1;
    }
    ...
    */
    ...
    // Create description
    let new_description = "Combined NFT of " + nft1.name + " and " + nft2.name;
    
    // Create new NonFungibleToken object
    let new_nft = NonFungibleToken {
        id: generate_uid::<NonFungibleToken>(),
        name: new_name,
        description: new_description,
        image: Url::from_utf8(new_image_url).unwrap(),
    };
    
    // Delete the two input NFTs
    delete::<NonFungibleToken>(&nft1);
    delete::<NonFungibleToken>(&nft2);
    
    // Transfer the new NFT to the sender
    transfer::transfer::<NonFungibleToken>(&get_sender(), &new_nft);
    ...
    */
    public fun withdraw_sales(
        ...
        // Check if caller is the owner of MinterCap
    if ctx.caller != minter_cap.id {
        abort("Only the owner can withdraw sales.");
    }
    
    // Transfer sales balance to the owner
    let withdrawn_amount = minter_cap.sales.value;
    minter_cap.sales.value = 0;
    let withdrawn_coin = Coin { value: withdrawn_amount };
    transfer::transfer_coin::<SUI>(&get_sender(), &withdrawn_coin);
    
    return withdrawn_coin;
    ...
    */
    ...
    if get_sender() != nft.id {
        abort("Only the owner can burn NFTs.");
    }
